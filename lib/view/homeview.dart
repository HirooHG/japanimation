                            ...
                            child: ChoiceChip(
                              ...
                              onSelected: (selected) {
                                if(state.currentCategory == category) {
                                  BlocProvider.of<JapanBloc>(context).add(SelectCategoryEvent(category: Category.empty()));
                                }
                                else {
                                  BlocProvider.of<JapanBloc>(context).add(SelectCategoryEvent(category: category));
                                }
                              },
              ...
              BlocBuilder<JapanBloc, JapanState>(
                builder: (_, state) {
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        Text((state.currentCategory.name == '') ? "All" : state.currentCategory.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20, fontFamily: "Ubuntu")),
                        IconButton(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ChoiceChip(
                            label: const Text("category", style: TextStyle(fontWeight: FontWeight.bold, fontFamily: "Ubuntu")),
                            selectedColor: const Color(0xFF42d392),
                            selected: state == "category",
                            onSelected: (selected) => {
                              if(selected) BlocProvider.of<ModifyJapanCubit>(context).change("category")
                            },
                          ),
                          ChoiceChip(
                            label: const Text("thing", style: TextStyle(fontWeight: FontWeight.bold, fontFamily: "Ubuntu")),
                            selectedColor: const Color(0xFF42d392),
                            selected: state == "thing",
                            onSelected: (selected) => {
                              if(selected) BlocProvider.of<ModifyJapanCubit>(context).change("thing")
                            },
                          )
                        ],
                      ),
                    ...
                    BlocBuilder<JapanBloc, JapanState>(
                      builder: (context, japanState){
                        ...
                        var categoriesStr = categories.map((e) => e.name).toList();
                        return SizedBox(
                          width: width,
                          child: Row(
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: Autocomplete<String>(
                                  fieldViewBuilder: (context, controllerField, focusNode, onFieldSubmitted) {
                                    return TextField(
                                          ...
                                          child: ListView.builder(
                                            ...
                                            itemBuilder: (BuildContext context, int index) {
                                              ...
                                              return GestureDetector(
                                                ...
                                                child: ListTile(
                                                  title: Text(option, style: const TextStyle(color: Colors.white, fontFamily: "Ubuntu")),
                                                ),
                                              );
                                            },
                                          ),
                                ...
                                child: TextButton(
                                  onPressed: () {
                                    if(controller.text.isNotEmpty) {
                                      if(state == "category") {
                                        BlocProvider.of<JapanBloc>(context).add(RemoveCategoryEvent(category: Category(name: controller.text)));
                                      }
                                      else {
                                        BlocProvider.of<JapanBloc>(context).add(RemoveThingEvent(thing: Thing(name: controller.text)));
                                      }
                            ...
                            Container(
                              ...
                              child: TextButton(
                                onPressed: () {
                                  if(controller.text.isNotEmpty) {
                                    if(state == "category") {
                                      BlocProvider.of<JapanBloc>(context).add(AddCategoryEvent(category: Category(name: controller.text)));
                                    }
                                    else {
                                      BlocProvider.of<JapanBloc>(context).add(AddThingEvent(thing: Thing(name: controller.text)));
                                    }
                                    Navigator.pop(context);
                                  }
                                },
